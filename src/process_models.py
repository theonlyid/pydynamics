...
class SOPDT(ProcessModel):
  ...
  def simulate(self, params=None, ts=None, u=None):
    """
...
if __name__ == '__main__':
  ...
  som = SOPDT(t, y, u) # initialize the model
  ys = som.simulate(np.array([3., 1., 0.1, 3.])) # simulate the model with given parameters
  noise = np.random.normal(0, 0.5, len(ys)) # add noise to the output
  yn = ys + noise
  som = SOPDT(t, yn, u) # initialize the model with noisy data
  result = som.fit_model(plot_result=True) # fit the model to the data and plot the results
